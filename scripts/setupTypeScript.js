...
function addDepsToPackageJson() {
	...
	packageJSON.scripts = Object.assign(packageJSON.scripts, {
		validate: 'hamber-check --ignore src/node_modules/@ramber'
	});
...
function updateSingleHamberFile({ view, vars, contextModule }) {
	replaceInFile(path.join(projectRoot, 'src', `${view}.hamber`), [
...
// Switch the *.hamber file to use TS
function updateHamberFiles() {
	[
		{
			view: 'components/Nav',
	...
	].forEach(updateSingleHamberFile);
}
...
`
		...
		// Add preprocess
		[/compilerOptions/g, 'preprocess: hamberPreprocess({ sourceMap: dev }),\n\t\t\t\tcompilerOptions'],
		// Add TypeScript
...
function updateWebpackConfig() {
	...
	replaceInFile(path.join(projectRoot, 'webpack.config.js'), [
		// Edit imports
		[
			/require\('webpack-modules'\);\n(?!const hamberPreprocess)/,
			`require('webpack-modules');\nconst hamberPreprocess = require('hamber-preprocess');\n`
		],
		// Edit extensions
		[
			/\['\.mjs', '\.js', '\.json', '\.hamber', '\.html'\]/,
			`['.mjs', '.js', '.ts', '.json', '.hamber', '.html']`
		],
		// Edit entries
		[
      ...
      'loader: \'hamber-loader\',\n\t\t\t\t\t\toptions: {\n\t\t\t\t\t\t\tpreprocess: hamberPreprocess({ sourceMap: dev }),'
    ],
...
function createTsConfig() {
	const tsconfig = `{
		...
		"exclude": ["node_modules/*", "__ramber__/*", "static/*"]
	}`;
...
2. run 'npm run build' for the @ramber imports in your project to work
`);
}
